...
struct StructureFromMotion {
  StructureFromMotion(double observed_x, double observed_y){
    ...
    this->camera_px = 320.0;
    this->camera_py = 240.0;
  }
  template <typename T>
  bool operator()(
                  const T* const cameras,
                  const T* const point3d,
                  T* residuals) const {
    //convert bal to putin extrinsic
    T rotvec[3] = {cameras[0], cameras[1], cameras[2]};
    T translation[3] = {cameras[3], cameras[4], cameras[5]};
    T rotation_transpose[9];
    T rotation[9];
    T distrotion[1] = {T(0.0)};
    T focal_length[1] = {cameras[6]};
    ceres::AngleAxisToRotationMatrix(rotvec,rotation_transpose); 
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
          rotation[i*3 + j] = rotation_transpose[j*3 + i];
        }
    }
  ...
  static ceres::CostFunction* Create(const double observed_x,
    ...
    return (new ceres::AutoDiffCostFunction<StructureFromMotion, 2, 9, 3>(
